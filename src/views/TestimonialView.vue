      ...
      <div class="textHome" v-if="testimonials[0].title && testimonials[0].description">
        <div>
          <!-- here we are actually displaying it when calling the array testimonials we refer to the first object which is 0 and then we use dot
            ...
            -->
          <h1 id="title" data-aos="zoom-in-up">{{ testimonials[0].title }}</h1>
        </div>
        <div>
          <h1 id="titleDescription" data-aos="zoom-in-up">{{ testimonials[0].description }}</h1>
        </div>
      </div>
...
-->
      <div class="grids">
        <div class="row">
          <div data-aos="zoom-in-up" class="col-md-4" v-for="info in testimonials.slice(1)" :key="info.fullname">
